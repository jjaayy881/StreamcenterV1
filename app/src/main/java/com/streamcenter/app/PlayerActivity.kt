...
class PlayerActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Ohne diesen Listener bleibt der Bildschirm bei einem Wiedergabefehler einfach
        // schwarz, ohne dass man (ausser per adb logcat) je erfaehrt, was los ist. Zusaetzlich:
        // auf manchen Geraeten (u.a. einigen Amazon-Modellen) liefert die Hardware-Dekodierung
        // zwar Frames an libvlc, aber die Anzeige auf der Surface schlaegt still fehl
        // ("window: request X not implemented" im Log) - dann automatisch einmal mit
        // Software-Dekodierung erneut versuchen, bevor wir einen echten Fehler anzeigen.
        mediaPlayer.setEventListener { event ->
            if (event.type == MediaPlayer.Event.EncounteredError) {
                if (!hwFallbackTried) {
                    infoText.showError("Hardware-Wiedergabe fehlgeschlagen - versuche Software-Dekodierung...")
                    infoText.visibility = TextView.VISIBLE
                    playIndex(currentIndex, useHwDecoder = false)
                } else {
                    infoText.showError("Wiedergabe fehlgeschlagen (Stream nicht erreichbar oder Format nicht unterstuetzt)")
                    infoText.visibility = TextView.VISIBLE
                }
            }
        }
    ...
    private var hwFallbackTried = false

    private fun playIndex(index: Int, useHwDecoder: Boolean = true) {
        currentIndex = index
        hwFallbackTried = !useHwDecoder
